#arnoldrimmer2210@gmail.com arniedoesitbest
...
def new_movie():
  if request.method == 'GET':
    ...
    if 'username' in login_session:
      ...
      category_id = session.query(Category).filter(Category.name == request.form['category']).one().id
      # return name + description + category + str(login_session['user_id'])
      new_movie = Movie(name=name, description=description, category_id=category_id, user_id=login_session['user_id'])
...
@app.route('/catalog/<string:category>/<int:movie_id>/edit', methods=['GET', 'POST'])
@app.route('/catalog/<string:category>/<int:movie_id>/edit/', methods=['GET', 'POST'])
def edit_movie(category, movie_id):
  if request.method == 'GET':
    ...
    if login_session.get('user_id') == movie.user_id:
      ...
      flash("You must be logged in as owner of " + movie.name + " to edit")
      return redirect(url_for('render_catalog'))
  else:
    try:
      movie = session.query(Movie).filter(Movie.id == movie_id).one()
    ...
    if login_session.get('user_id') == movie.user_id:
      ...
      movie.category_id = session.query(Category).filter(Category.name == request.form['category']).one().id
      session.commit()
...
@app.route('/catalog/<string:category>/<int:movie_id>/delete', methods=['GET', 'POST'])
@app.route('/catalog/<string:category>/<int:movie_id>/delete/', methods=['GET', 'POST'])
def delete_movie(category, movie_id):
  ...
  if request.method == 'GET':
    if login_session.get('user_id') == movie.user_id:
      ...
      flash('You must be logged in as owner of ' + movie.name + 'to delete')
      return redirect(url_for('render_catalog'))
  else:
    if request.form['submit'] == 'cancel':
...
def gconnect():
  ...
  if result['user_id'] != gplus_id:
    response = make_response(json.dumps('Token\'s user ID doesn\'nt match the given user id'), 401)
    response.headers['Content-Type'] = 'application/json'
    return response
